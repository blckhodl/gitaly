...
func TestObjectReader_queue(t *testing.T) {
	...
	t.Run("read single object", func(t *testing.T) {
		...
		require.NoError(t, queue.RequestRevision(ContentsCommand, foobarBlob.Revision()))
	...
	t.Run("read multiple objects", func(t *testing.T) {
		...
		for blobID, blobContents := range map[git.ObjectID]string{
			...
			require.NoError(t, queue.RequestRevision(ContentsCommand, blobID.Revision()))
			require.NoError(t, queue.Flush())
	...
	t.Run("request multiple objects", func(t *testing.T) {
		...
		require.NoError(t, queue.RequestRevision(ContentsCommand, foobarBlob.Revision()))
		require.NoError(t, queue.RequestRevision(ContentsCommand, barfooBlob.Revision()))
		require.NoError(t, queue.Flush())
	...
	t.Run("request invalid object", func(t *testing.T) {
		...
		require.NoError(t, queue.RequestRevision(ContentsCommand, "does-not-exist"))
		require.NoError(t, queue.Flush())
	...
	t.Run("closing queue blocks request", func(t *testing.T) {
		...
		require.Equal(t, fmt.Errorf("cannot request revision: %w", os.ErrClosed), queue.RequestRevision(ContentsCommand, foobarBlob.Revision()))
	})
