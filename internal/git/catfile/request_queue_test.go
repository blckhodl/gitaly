...
func TestRequestQueue_ReadObject(t *testing.T) {
	...
	t.Run("read on closed reader", func(t *testing.T) {
		...
		require.NoError(t, queue.RequestRevision(ContentsCommand, "foo"))
...
func TestRequestQueue_RequestRevision(t *testing.T) {
	...
	t.Run("requesting revision on closed queue", func(t *testing.T) {
		...
		require.Equal(t, fmt.Errorf("cannot request revision: %w", os.ErrClosed), queue.RequestRevision(ContentsCommand, "foo"))
	})
	...
	t.Run("multiple request", func(t *testing.T) {
		...
		require.NoError(t, queue.RequestRevision(ContentsCommand, "foo"))
		require.NoError(t, queue.RequestRevision(ContentsCommand, "bar"))
		require.NoError(t, queue.RequestRevision(ContentsCommand, "baz"))
		require.NoError(t, queue.RequestRevision(ContentsCommand, "qux"))
		require.NoError(t, queue.Flush())
	...
	t.Run("multiple request with intermediate flushing", func(t *testing.T) {
		...
		for _, revision := range []git.Revision{
			...
			require.NoError(t, queue.RequestRevision(ContentsCommand, revision))
			require.NoError(t, queue.Flush())
