...
func TestObjectInfoReader_queue(t *testing.T) {
	...
	t.Run("read single info", func(t *testing.T) {
		...
		require.NoError(t, queue.RequestRevision(InfoCommand, blobOID.Revision()))
	...
	t.Run("read multiple object infos", func(t *testing.T) {
		...
		for oid, objectInfo := range map[git.ObjectID]ObjectInfo{
			...
			require.NoError(t, queue.RequestRevision(InfoCommand, oid.Revision()))
			require.NoError(t, queue.Flush())
	...
	t.Run("request multiple object infos", func(t *testing.T) {
		...
		require.NoError(t, queue.RequestRevision(InfoCommand, blobOID.Revision()))
		require.NoError(t, queue.RequestRevision(InfoCommand, commitOID.Revision()))
		require.NoError(t, queue.Flush())
	...
	t.Run("request invalid object info", func(t *testing.T) {
		...
		require.NoError(t, queue.RequestRevision(InfoCommand, "does-not-exist"))
		require.NoError(t, queue.Flush())
	...
	t.Run("closing queue blocks request", func(t *testing.T) {
		...
		require.Equal(t, fmt.Errorf("cannot request revision: %w", os.ErrClosed), queue.RequestRevision(InfoCommand, blobOID.Revision()))
	})
