...
func TestSubtransaction_isQuorumPossible(t *testing.T) {
	voteA := newVote(t, "a")
	voteB := newVote(t, "b")
	voteC := newVote(t, "c")
	...
	for _, tc := range []struct {
		...
		{
			desc: "Single vote and single failure",
			voters: []Voter{
				{Name: "1", Votes: 1, vote: &voteA},
				{Name: "2", Votes: 1, vote: nil},
				{Name: "3", Votes: 1, vote: nil},
			},
			failedNodes:    map[string]struct{}{"2": {}},
			threshold:      2,
			quorumPossible: true,
		},
		{
			desc: "Two different votes and no failure",
			voters: []Voter{
				{Name: "1", Votes: 1, vote: &voteA},
				{Name: "2", Votes: 1, vote: &voteB},
				{Name: "3", Votes: 1, vote: nil},
		...
		{
			desc: "Two different votes and one failure",
			voters: []Voter{
				{Name: "1", Votes: 1, vote: &voteA},
				{Name: "2", Votes: 1, vote: &voteB},
				{Name: "3", Votes: 1, vote: nil},
			},
			failedNodes:    map[string]struct{}{"3": {}},
			threshold:      2,
			quorumPossible: false,
		},
		{
			desc: "Three different votes and no failure",
			voters: []Voter{
				{Name: "1", Votes: 1, vote: &voteA},
				{Name: "2", Votes: 1, vote: &voteB},
				{Name: "3", Votes: 1, vote: &voteC},
		...
		t.Run(tc.desc, func(t *testing.T) {
			...
			for _, v := range tc.voters {
				if v.vote != nil {
					s.voteCounts[*v.vote] += v.Votes
				}
			}

			require.Equal(t, tc.quorumPossible, s.isQuorumPossible(tc.failedNodes))
		})
	}
}
